...
void UJumpState::TickState(UCustomCharacterMovementComponent* MovementComponent, float DeltaTime)
{
	...
	if (MovementComponent->GetState((ECustomMovementMode::MOVE_Previous)) == MovementComponent->GetState((ECustomMovementMode::MOVE_Sprint)) &&
		MovementComponent->CanJumpOverObstacle())
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Green, TEXT("Jump Over Obstacle Success"));
		MovementComponent->ChangeState(MovementComponent->GetState(ECustomMovementMode::MOVE_JumpOverObstacle));
	}
	if (MovementComponent->EnableWallRun && MovementComponent->CanWallRun(MovementComponent->wallDirection) &&
		MovementComponent->prevWallDirection != MovementComponent->wallDirection) { // Add Condition for Run, but not just jump;
